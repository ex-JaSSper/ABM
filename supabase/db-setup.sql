-- =====================================================================
--  ABM-сервис · РАЗОВАЯ НАСТРОЙКА БАЗЫ для Supabase (режим MVP без логина)
--  Запустить ЦЕЛИКОМ: Supabase → SQL Editor → New query → вставить → Run.
--  Создаёт все таблицы + открывает доступ по публичному (anon) ключу.
--  Перед реальным запуском включим авторизацию (заменим anon-политики).
-- =====================================================================
...
create table if not exists company (
  ...
  segment text default '', category text default '', geography text default '',
  why_fit text default '', signal_note text default '', relevant_hypothesis_text text default '',
  what_to_check text default '', who_to_find text default '', extended_reason text default '',
  first_message_template text default '', second_message_template text default '',
...
create table if not exists contact (
  ...
  ext_company_no text default '', company_name text default '',
  priority text default '', segment text default '', role_target text default '',
  contact_no text default '', full_name text not null default '', position text default '',
  phone text default '', email text default '', telegram text default '', tenchat text default '',
  network text default '', linkedin text default '', other_social text default '', source text default '',
  last_digital_trace text default '', last_trace_date text default '', check_date text default '',
  confidence text default '', comment text default '', how_to_get text default '',
...
-- ---------- RLS: доступ по публичному ключу (MVP без логина) ----------
...
-- ---------- Стартовая пустая стратегия + её KPI ----------
